...
public class PostofficeImpl implements IPostOffice {
	...
	public PostOfficeResponseBean fetchPostOfficeDetailsByCity(String city) {
		...
		try {
			ResponseEntity<PostOfficeResponseBean[]> cityResponseEntity = restTemplate.getForEntity(url,
					PostOfficeResponseBean[].class);
			PostOfficeResponseBean[] cityResponseBeans = cityResponseEntity.getBody();
			if (cityResponseBeans != null && cityResponseBeans.length > 0) {
			...
			logger.warning("An error occurred while fetching post office details by city: " + e.getMessage());
		}
		return null;
	}
	...
	public PostOfficeResponseBean fetchPostOfficeDetailsByPincode(String pincode) {
		...
		try {
			ResponseEntity<PostOfficeResponseBean[]> pincodeResponseEntity = restTemplate.getForEntity(url,
					PostOfficeResponseBean[].class);
			PostOfficeResponseBean[] pinodeResponseBeans = pincodeResponseEntity.getBody();
			if (pinodeResponseBeans != null && pinodeResponseBeans.length > 0) {
